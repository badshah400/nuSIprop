...
public:
  ...
  void evolve(void){
	...
	Emin_i = Emin[i];
	Emax_i = Emax[i];
      } else{ // Auxiliary values for highest bins at highest redshifts
...
private:
  ...
  double flux_FS_E0(){
	...
	z3 = (b-a)/2. * nuSIaux::x_integ[2] + (b+a)/2.;
      ...
      res += (b-a)/2. * (nuSIaux::w_integ[0] * pow(1+z1, -si) * get_SFR(z1) / get_H(z1) +
			 nuSIaux::w_integ[1] * pow(1+z2, -si) * get_SFR(z2) / get_H(z2) +
			 nuSIaux::w_integ[2] * pow(1+z3, -si) * get_SFR(z3) / get_H(z3) );
    }
    return res;
  }
  ...
  double energy_FS(double z_min, double z_max){
	...
	z3 = (b-a)/2. * nuSIaux::x_integ[2] + (b+a)/2.;
      ...
      res += (b-a)/2. * (nuSIaux::w_integ[0] * Lum_times_E(z1, pow(10, lEmin), pow(10, lEmax))/get_H(z1) +
			 nuSIaux::w_integ[1] * Lum_times_E(z2, pow(10, lEmin), pow(10, lEmax))/get_H(z2) +
			 nuSIaux::w_integ[2] * Lum_times_E(z3, pow(10, lEmin), pow(10, lEmax))/get_H(z3));
    }
    return res;
  }
   ...
   */
	...
	( 2 * log1p(splus)/splus - 2 * log1p(sminus)/sminus
	  + log1p(splus) - log1p(sminus) );
      if(Gamma_t_u < 0){ // Roundoff errors! Compute the integral numerically
	// Limits of the integral
	double a = sminus;
	double b = splus;
	// Nodes at which the integrand will be evaluated
	...
	(creal(dilogdiff_z1) + creal(dilogdiff_z2)
	 + ga_red*( cimag(dilogdiff_z2) - cimag(dilogdiff_z1) )
	 + 2*ga_red*carg(1 - z2_plus)*log1p(splus) - 2*ga_red*carg(1 - z2_minus)*log1p(sminus)
	 + log1p(4/SQR(ga_red)) * (log1p(sminus) - log1p(splus))
	 + log1p(SQR(-1 + splus)/SQR(ga_red))*log1p(splus)
	 - log1p(SQR(-1 + sminus)/SQR(ga_red))*log1p(sminus)
	 + (1 + SQR(ga_red)) * (log1p(SQR(-1 + sminus)/SQR(ga_red))
				- log1p(SQR(-1 + splus)/SQR(ga_red)))
	 + 2*nuSIaux::dilogdiff(splus, sminus));
      // Prefactor: |U_{flav i}|^2 |U_{flav j}|^2 \sum_{kl} |U_{flav k}|^2 |U_{flav l}|^2
  ...
  double alphaTilde(double Em, double Ep){
	...
	if(alphaTilde_t < 0){ // Roundoff errors! Compute the integral numerically
	...
	alphaTilde_u = 1./2. * SQR(SQR(g)) / (32*SQR(SQR(mphi))*M_PI*(-1 + tminus)*tplus) *
	  ((-2 + tminus)*(tminus - tplus) -
	   (-1 + tminus)*(-2 + tplus)*(log1p(-tminus) - log1p(-tplus)));
	if(alphaTilde_u < 0){ // Roundoff errors! Compute the integral numerically
	  double a_y = tplus, b_y = tminus, a_x[3], b_x[3];
	  // Nodes at which the integrand will be evaluated
	  double y[3], x[3][3], F[3][3];
	  alphaTilde_u = 0;
	...
	alphaTilde_tu = 0;
      ...
      if (-tplus < 1e-5){ // We Taylor-expand dilogdiff to be fast and avoid roundoff errors
	double delta = tplus/tminus;
	...
	alphaTilde_st = SQR(SQR(g)) / (32*M_PI*(1 + SQR(ga_red))*SQR(SQR(mphi))) *
	  (2*M_PI*carg(-1 + I*ga_red - tminus) - 2*M_PI*carg(-1 + I*ga_red - tplus)
	   + 2*ga_red*(cimag(dilogdiff_z5z1) + cimag(dilogdiff_z2z6) + cimag(dilogdiff_z4z3))
	   - 2*(creal(dilogdiff_z5z1) + creal(dilogdiff_z2z6) + creal(dilogdiff_z4z3) + creal(dilogdiff_z7z8))
	   - carg((ga_red + I*(1 + tminus))/(2*I + ga_red))*(2*M_PI + 2*ga_red*log1p(-tminus))
	   + carg((ga_red + I*(1 + tplus))/(2*I + ga_red))*(2*M_PI + 2*ga_red*log1p(-tplus))
	   + (carg(-1 + I*ga_red - tminus) - carg(-1 + I*ga_red - tplus))*(4*ga_red*tminus + 2*ga_red*log1p(-tminus))
	   + 2*ga_red*(carg(1 + tminus) - carg(2 - I*ga_red + tminus) + carg(1 - I*ga_red + tplus))*log1p(tminus - tplus)
	   + log(4 + SQR(ga_red))*(log1p(-tplus) - log1p(-tminus))
	...
	alphaTilde_st = SQR(SQR(g)) / (32*M_PI*(1 + SQR(ga_red))*SQR(SQR(mphi)))*
	  (ga_red*cimag(dilogdiff_z5z1)
	   - 2*(creal(dilogdiff_z5z1 + dilogdiff_z7z8))
	   + 2*carg((ga_red + I*(1 + tminus))/(2*I + ga_red))*(-M_PI - ga_red*log1p(-tminus))
	   + 2*carg(-1 + I*ga_red - tminus)*(M_PI + ga_red*tminus + ga_red*log1p(-tminus))
	   - 2*carg(-1 + I*ga_red - tplus)*(M_PI + ga_red*tminus + ga_red*log1p(-tminus))
	   + 2*carg((ga_red + I*(1 + tplus))/(2*I + ga_red))*(M_PI + ga_red*log1p(-tplus))
	   - 2*log1p(-tminus)*log(-tplus)
	   + 2*tminus*log(tminus/tplus)
	   + 2*log1p(-tplus)*log(-tplus)
	   + (log1p(-tplus) - log1p(-tminus))*(log(4+SQR(ga_red)) - 2*log(ga_red) - log1p(SQR(1+tplus)/SQR(ga_red)))
	   + (1 + tminus + SQR(ga_red))*(log1p(SQR(1 + tplus)/SQR(ga_red)) - log1p(SQR(1 + tminus)/SQR(ga_red)))
	   );
	...
	if(majorana) // For Majorana fermions, all final states are observable
      ...
      if(alphaTilde_s < 0 || alphaTilde_t<0 || alphaTilde_u < 0
	 || alphaTilde_tu / SQR(SQR(g/mphi)) < -1e-11
  ...
  double alpha(double Em, double Ep, double Em_prime, double Ep_prime){
	...
	alpha_st = SQR(SQR(g)) / (32*M_PI*(1 + SQR(ga_red))*SQR(SQR(mphi))) *
	  (2*ga_red*(dilog_z1_im.val - dilog_z2_im.val - dilog_z3_im.val + dilog_z4_im.val
		     - dilog_z5_im.val + dilog_z6_im.val + dilog_z7_im.val - dilog_z8_im.val)
	   - 2*(dilog_z1_re.val - dilog_z2_re.val - dilog_z3_re.val + dilog_z4_re.val
		- dilog_z5_re.val + dilog_z6_re.val + dilog_z7_re.val - dilog_z8_re.val)
	   + 2*ga_red*(carg(-(1/(1 + tminus))) - carg(-((-1 + I*ga_red + sminus_prime)/(2 - I*ga_red + tminus))))*log1p(sminus_prime + tminus)
	   - 2*ga_red*(carg(-(1/(1 + tminus))) - carg(-((-1 + I*ga_red + splus_prime)/(2 - I*ga_red + tminus))))*log1p(splus_prime + tminus)
	   ...
	   + log1p(splus_prime + tplus)*(log1p(SQR(-1 + splus_prime)/SQR(ga_red)) - log1p(SQR(2 + tplus)/SQR(ga_red)) - 2*(log(splus_prime) - log(fabs(1 + tplus))))
	   );
      } else{
	alpha_st = SQR(SQR(g)) / (32*M_PI*(1 + SQR(ga_red))*SQR(SQR(mphi))) *
	  (
	   (2*ga_red*carg(-1 + I*ga_red + sminus_prime) - 2*ga_red*carg(-1 + I*ga_red + splus_prime)
	    + 2*log(sminus_prime) - 2*log(splus_prime)
	    + log1p(SQR(-1 + splus_prime)/SQR(ga_red)) - log1p(SQR(-1 + sminus_prime)/SQR(ga_red)) )*
	...
	if(majorana) // For Majorana fermions, all final states are observable
      ...
      if(alpha_s < 0 || alpha_t / SQR(SQR(g/mphi)) < -1e-11 || alpha_u / SQR(SQR(g/mphi)) < -1e-11
	 || alpha_tu / SQR(SQR(g/mphi)) < -1e-11
	 || (alpha_st + alpha_s + alpha_t) / SQR(SQR(g/mphi)) < -1e-11){
